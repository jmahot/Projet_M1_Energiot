...
def generer_events_appareil(appareil_id, start_dt, jours, etat_initial="OFF"):
    ...
    for jour in range(jours):
        ...
        # Générer events journée avec alternance ON/OFF
        t = jour_debut
        ...
        while t < jour_fin:
            ...
            # Ajouter event uniquement si état différent du dernier
        ...
        # Générer events nuit toujours OFF entre 22h-23h
        ...
        # Générer events nuit toujours OFF entre 0h-6h
        for _ in range(random.randint(0, 2)):
        ...
        # Trier par date_complete
        temp_events.sort(key=lambda x: x["date_complete"])

        # Mettre à jour état final pour passer au jour suivant
        etat = dernier_etat

        # Ajouter à la liste globale des events
...
with open("Database/database_energiot_v2.json", "w") as f:
